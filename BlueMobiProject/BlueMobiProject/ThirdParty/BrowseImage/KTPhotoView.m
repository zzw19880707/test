...
{
    float iw = newImage.size.width;
    float ih = newImage.size.height;
    ...
    if (ih/iw > sh/sw) {
        w = sh*(iw/ih);
        h = sh;
        imageView_.frame = CGRectMake((sw-w)/2, 0, w, h);
    } else {
        w = sw;
        h = sw*(ih/iw);
        imageView_.frame = CGRectMake(0, (sh-h)/2, w, h);
    }
...
{
    [imageView_ setImageWithURL:[NSURL URLWithString:[url stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]] placeholderImage:nil];
}
...
-(void)scrollViewDidZoom:(UIScrollView *)scrollView{
    ...
    CGFloat xcenter = scrollView.bounds.size.width/2,ycenter = scrollView.bounds.size.height/2;
...
//    }
    [imageView_ setCenter:CGPointMake(xcenter, ycenter)];
}
